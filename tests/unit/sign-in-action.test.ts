...
vi.mock("../../src/server/auth/session", () => ({ setSessionCookie: mocks.setSessionCookieMock }));
...
const NEON_OK = (user: { id: string; email: string }) =>
...
describe("signInAction owner allowlist (PR #2d verification-fix CRITICAL #2)", () => {
  ...
  it("upserts the owner and sets the session cookie when credentials match OWNER_USER_ID/OWNER_EMAIL", async () => {
    ...
    expect(mocks.setSessionCookieMock).toHaveBeenCalledWith("tok-xyz");
  });
